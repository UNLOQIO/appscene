...
function doRequest(opt, done) {
  request(opt, function(err, res, body) {
    if(err) {
      return done(util.error('SERVER_ERROR', 'Failed to contact servers.', opt.uri));
    }
    ...
    if(body.type !== 'success') {
      return done(util.error(body.code || 'SERVER_ERROR', body.message || 'Failed to contact servers.', opt.uri));
    }
    done(null, body.data || {});
  });
}
